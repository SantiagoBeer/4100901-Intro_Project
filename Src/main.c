    ...
    if (button_flag == 1)
{
  ...
  // 2. Encender LED Interno (LD2)
  HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET); // Enciende LD2

  // 3. Calcular cuándo apagar (ambos) después de 1 segundo
  led_ext_off_time = HAL_GetTick() + 2000;

  // 4. Enviar mensaje por UART
  sprintf(tx_buffer, "CONCEDER_ACCESO.\r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
{
  ...
  // 2. Enviar mensaje por UART
  sprintf(tx_buffer, "CERRAR_SISTEMA.\r\n");
  HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);

  // 3. Resetear el tiempo de apagado
  led_ext_off_time = 0;
}
